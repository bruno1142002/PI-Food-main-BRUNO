...
const { YOUR_API_KEY } = '1f7a92bb9fe946df85c2e1df498b1448' ; // resolver problema de API//
...
if(id.length<10){
    ...
    const receta = {
        id: ee.id,
          ...
          healthScore: ee.healthScore,
          spoonacularScore: ee.spoonacularScore,
          analyzedInstructions: (ee.analyzedInstructions.length > 0) ? ee.analyzedInstructions[0].steps.map(e => e.step) : ["No se encontraron datos"]
...
route.post('/', async (req, res) => {
    ...
    else {return res.status(404).setDefaultEncoding('Error')}
})
